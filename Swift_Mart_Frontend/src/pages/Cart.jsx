...
function Cart() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/view-card-product", {
        method: "get",
        credentials: "include",
        headers: {
      ...
      if (responseData.success) {
        ...
        // console.log("Cart data loaded:", responseData.data.length, "items");
      } else {
  ...
  // Delete product from cart (fixed: uses productId)
  const deleteCartProduct = async (id) => {
    try {
      ...
      const response = await fetch("http://localhost:8080/api/delete-cart-product", {
        ...
        body: JSON.stringify({ productId: id }), // <-- Fixed here
      });
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // console.log("Sending cart data to checkout:", data);
                ...
                >
                  <div className="w-32 h-32 bg-blue-50 dark:bg-blue-800 rounded-l-xl">
                    <img
                      ...
                      className="w-full h-full object-scale-down p-2 bg-white rounded-lg" // <-- Fixed: removed mix-blend-multiply, added bg-white
