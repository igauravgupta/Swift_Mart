...
const OlxMarketplace = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/olx/products?${queryParams.toString()}`, {
        credentials: "include",
      })
  ...
  const fetchMyProducts = async () => {
    try {
      const response = await fetch("http://localhost:8080/api/olx/my-products", {
        credentials: "include",
      })
