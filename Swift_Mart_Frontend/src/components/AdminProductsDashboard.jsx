...
const AdminProductsDashboard = () => {
  ...
  const fetchCounts = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/olx/admin-product-counts`, {
        credentials: 'include'
      });
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/olx/admin-products?status=${filter}`, {
        credentials: 'include'
      });
  ...
  const handleQuickAction = async (productId, status) => {
    try {
      const response = await fetch('http://localhost:8080/api/olx/update-product-status', {
        method: 'POST',
        credentials: 'include',
        headers: {
