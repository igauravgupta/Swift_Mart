...
const Header = ({ cartCount, olxCartCount, user, isDark, toggleTheme }) => {
  ...
  // Fetch user details and OLX cart count on mount
  useEffect(() => {
    if (!reduxUser?._id) {
    ...
    <>
      <header className="w-full h-16 shadow-md bg-white/90 dark:bg-[#101624] backdrop-blur-md fixed z-40 top-0 left-0 transition-colors duration-300">
        <div className="h-full max-w-7xl mx-auto flex items-center px-4 justify-between">
          ...
          {/* Search Bar */}
          <div className="hidden lg:flex items-center w-full max-w-sm border border-blue-200 dark:border-blue-800 rounded-full focus-within:shadow-lg pl-2 bg-white dark:bg-[#1b2336] transition-all duration-300">
            <input
              type="text"
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            {reduxUser?._id && (
              <>
                <Link
                  to="/olx-marketplace"
                  className="flex items-center gap-1 px-3 py-1 rounded-full bg-blue-600 hover:bg-blue-700 text-white transition-all"
                >
                  <FaStore className="text-sm" />
                  <span className="hidden md:block">OLX Market</span>
                </Link>
                ...
                <Link
                  to="/mystery-box"
                  className="px-3 py-1 flex items-center gap-2 rounded-full text-white bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 transform transition duration-300 ease-in-out hover:scale-105 shadow"
                >
                  <MysteryBoxIcon />
                  <span className="hidden lg:block">Mystery Box</span>
                </Link>
              ...
              {reduxUser?._id ? (
                ...
                >
                  {reduxUser.profilePic ? (
                    <img
                      src={`http://localhost:8080${reduxUser.profilePic}` || "/default-profile.png"}
                      alt="Profile"
                ...
                <div className="absolute bg-white dark:bg-[#1b2336] top-11 right-0 p-2 shadow-lg rounded z-50 w-52 border border-blue-100 dark:border-blue-800">
                  <nav className="flex flex-col text-sm">
                    ...
                    {isAdmin && (
                      <>
                        <Link
                          ...
                          className="hover:bg-blue-50 dark:hover:bg-blue-900 p-2 rounded transition"
                        >
                          Admin Panel
                        </Link>
                        <Link
                          to="/admin-panel/admin-products"
                          className="hover:bg-blue-50 dark:hover:bg-blue-900 p-2 rounded transition"
                        >
                          OLX Approvals
                        </Link>
                    ...
                    <button
                      ...
                      className="text-left hover:bg-blue-50 dark:hover:bg-blue-900 p-2 rounded transition"
                    >
            ...
            <div>
              {reduxUser?._id ? (
                <button
                  ...
                  className="px-3 py-1 rounded-full text-white bg-blue-600 hover:bg-blue-700 transition"
                >
                  Logout
                </button>
              ) : (
